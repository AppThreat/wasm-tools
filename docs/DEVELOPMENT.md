...
User-facing documentation lives in `docs/` and is published to GitHub Pages with docsify (no build step; `.github/workflows/docs.yml` deploys the folder). If you add a page, register it in `docs/_sidebar.md`. Lessons follow the naming convention `LESSON<N>.md`, use repo fixtures so every command is reproducible, and avoid em-dashes and emoji by project convention. The in-repo map in `AGENTS.md` mirrors this structure; update it when pages move.

The examples are guarded against drift: `tests/test_docs.py` extracts every fenced bash block in `docs/` that references `tests/fixtures/` and runs it from the repository root. If a code change breaks a documented command, that test fails with the page and block number. Blocks needing tools the environment lacks (`wat2wasm`, `jq`, `xxd`) or scripts the page tells the reader to write are skipped with the reason recorded, so keep runnable examples pointed at committed fixtures.
