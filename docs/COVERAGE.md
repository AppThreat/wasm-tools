...
| Type section               | `5.4-binary.modules.spectec` | Tested  | Including GC subtype and rec-type wrappers.                                             |
...
| Exception handling                                         | `5.3-binary.instructions.spectec` | Tested | `throw`, `throw_ref`, `try_table` with full catch lists.                             |
...
| CLI headers mode (`--headers`)   | Tested          | Section table output.                                                   |
...
| Text format (`.wat`) input       | Not implemented | Handled externally by WABT's `wat2wasm` for fixture builds.             |
